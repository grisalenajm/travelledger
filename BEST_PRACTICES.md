...
| Fix 58 | Monedas duplicadas en 3 schemas backend + 6 arrays hardcodeados distintos en frontend (ninguno con MYR). Fuente única: `backend/app/core/currencies.py` con `CURRENCY_NAMES` (dict ISO→nombre ES) y `VALID_CURRENCIES = frozenset(CURRENCY_NAMES)` — **61 monedas**, prioridad Sudeste Asiático. `schemas/expense.py`, `schemas/trip.py` y `schemas/auth.py` (alias `_VALID_CURRENCIES`) importan en vez de redefinir. Nuevo `GET /api/currencies/list` → `list[CurrencyOption]` (`code`+`name`), protegido con `get_current_user`, sin trailing slash. Frontend: hook `use-currencies.ts` (React Query, `staleTime` 24h) sustituye los 6 arrays en 7 ficheros / 8 `<select>`. Los selects pasan a **controlados** (`value={watch("campo")}` junto al `{...register}`): con opciones que llegan async, un select sin `value` deja `selectedIndex=0` al insertarse los `<option>` y muestra una moneda distinta de la del formulario. `COMMON_CURRENCIES` de `routers/currencies.py` se deja intacto — solo alimenta `/rates`, ningún componente lo consume. Sin migración Alembic. |
| Fix 64 | `PaymentMethodCombobox`: `autoFocus` en input de búsqueda robaba el foco del trigger → disparaba `onBlur` → `setTimeout(150ms)` cerraba el dropdown antes de que el tap en una opción se registrara (especialmente en móvil/touch, en `/expenses/scan/confirm`). El `<select>` nativo de `trips/[id]/expenses/[expenseId]` no tenía el bug porque usa el picker nativo del SO sin lógica de blur propia. Fix: eliminar `autoFocus` + `onBlur`/`setTimeout`, sustituir por listener `pointerdown` en `document` (patrón click-outside estándar) con `ref` al contenedor; foco del input de búsqueda ahora manual vía `useEffect` + `inputRef`. |
| Fix 65 | `location_name` quedaba `None` para siempre cuando el EXIF de la foto SÍ traía coords GPS (`receipts.py`, comentario "no tenemos nombre textual del GPS" nunca resuelto). `geocoding_service.reverse_geocode(lat,lng)` ya existía pero no estaba conectado. Fix: `reverse_geocode_expense_bg()` en `expense_service.py` (patrón idéntico a `geocode_expense_bg`, sesión propia `AsyncSessionLocal`), encolado como BackgroundTask solo cuando `location_lat/lng` vienen de EXIF; no toca lat/lng, solo `location_name`. `reverse_geocode()` devuelve el JSON crudo de Nominatim (**no** trae una clave `name` fiable) — nombre corto derivado con `result.get("name") or result.get("display_name", "").split(",")[0].strip()`, mismo patrón ya usado en `routers/geocoding.py`. Verificado en producción contra Nominatim real (coords Torre Eiffel → `location_name="Avenue Gustave Eiffel"`, lat/lng sin tocar). No cubre el caso — mucho más común en Android — de fotos sin GPS EXIF en absoluto (limitación de la app de cámara nativa, no del código); ese caso sigue dependiendo del fallback frágil de geocodificar por `ocr.description`. |
| Fix 66 | `generate_xlsx()` no tenía columna que correlacionara cada fila de gasto con su fichero de imagen dentro del bundle ZIP (`build_csv()`, usado en el bundle CSV, sí la tenía vía `image_file`). Fix: parámetro opcional `image_map: dict[str,str] \| None = None` en `generate_xlsx()` (mismo patrón que `payment_methods`), columna "Imagen/Documento" añadida al final solo cuando se invoca desde `build_bundle()` con `format="xlsx"`. El export suelto `/export/{id}?format=xlsx` no cambia — sigue sin columna de imagen, ya que ahí no hay ZIP ni ficheros que correlacionar. Renombrado de "Fix 63" (numeración del prompt original, basada en el máximo de `git log` en el momento de escribirlo) a Fix 66 porque en esta misma sesión ya se habían hecho los Fix 64 y 65 — mismo criterio de renumeración que el propio prompt aplicó sobre su antecesor "Fix 58". |
