...
async def get_trip_summary(
    ...
    trip = await get_trip_or_404(db, trip_id, current_user.id)
    expenses = await expense_service.list_expenses(db, current_user.id, trip_id=trip_id)
...
async def export_trip(
    ...
    cards = await loyalty_card_service.list_cards(db, current_user.id)
