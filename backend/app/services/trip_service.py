...
async def get_active_trip(db: AsyncSession, user_id: UUID) -> Trip | None:
    """Devuelve el viaje cuyo rango de fechas incluye hoy y status=active.

    Si hay varios, devuelve el que empezó más recientemente.
...
async def get_summary(db: AsyncSession, trip_id: UUID, user: User, *, effective_user_id: UUID | None = None) -> TripSummary:
    uid = effective_user_id if effective_user_id is not None else user.id
    trip = await get_or_404(db, trip_id, uid)
