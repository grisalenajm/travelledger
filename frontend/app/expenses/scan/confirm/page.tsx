    ...
    return (
      <main className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center px-6">
          ...
          <button
            type="button"
            onClick={() => router.push(tripId ? `/trips/${tripId}` : "/trips")}
            ...
            {receiptProxyUrl ? (
              receiptContentType?.startsWith("image/") ? (
                <div className="rounded-xl overflow-hidden border border-outline-variant/20 shadow-[0_8px_32px_rgba(26,28,30,0.06)]">
                  {receiptObjectUrl ? (
                    <img
                      ...
                      style={{ maxHeight: "500px", filter: "grayscale(20%)" }}
                    />
                  ) : (
                ...
                <a
                  ...
                  className="flex flex-col items-center justify-center h-64 rounded-xl border-2 border-dashed border-outline-variant text-on-surface-variant hover:border-primary hover:text-primary transition-colors"
                >
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {CATEGORIES.map((cat) => (
                <button
                  ...
                  className={[
                    "px-4 py-1.5 rounded-full text-xs font-label font-bold transition-colors",
                    category === cat
      ...
      <footer className="fixed bottom-0 inset-x-0 z-20 bg-surface/80 backdrop-blur-xl border-t border-outline-variant/20">
        <div className="max-w-screen-xl mx-auto px-6 py-4 flex items-center justify-between">
          <button
            ...
            onClick={() => router.back()}
            disabled={isPending}
            className="text-sm font-medium text-on-surface-variant hover:text-on-surface transition-colors disabled:opacity-50 px-1"
          >
            Cancelar
          </button>
          <button
