...
export const authOptions: AuthOptions = {
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          const role = user.is_guest ? "guest" : user.is_admin ? "admin" : "user"
          return {
            id: user.id,
            email: user.email,
            name: user.name,
  ...
  callbacks: {
    async jwt({ token, user }) {
      ...
      if (user) {
        return {
          ...
          accessTokenExpires: Date.now() + ACCESS_TOKEN_TTL_MS,
          isGuest: (user as { isGuest?: boolean }).isGuest ?? false,
          role: ((user as { role?: "admin" | "user" | "guest" }).role) ?? "user",
          mustChangePassword:
    ...
    async session({ session, token }) {
      ...
      session.user.role = (token.role ?? "user") as "admin" | "user" | "guest"
